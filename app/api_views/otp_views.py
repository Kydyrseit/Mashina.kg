...
class RequestOTPView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # Симулируем отправку OTP (реально здесь можно подключить SMS или Telegram)
...
class VerifyOTPView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # Получаем сохраненный OTP из кэша
        otp_stored = cache.get(f"otp_{phone}")
        if not otp_stored:
            return Response({"error": "OTP expired or not found."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        # OTP подтвержден — получаем или создаем пользователя с указанным номером телефона.
        # Для простоты используем номер телефона как username.
        user, created = User.objects.get_or_create(username=phone, defaults={"is_active": True})
